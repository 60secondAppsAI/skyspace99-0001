...
<template>
        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <h5 class="card-title">Weathers</h5>
              
              <div>
                  <base-button class="btn btn-primary" @click="modalWeathers = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalWeathers">
                <template slot="header">
  ...
  <base-input label="Temperature" type="text" placeholder="Enter Temperature" v-model="weatherToAdd.temperature"></base-input>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="weathers" :row-key="record => record.WeatherId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <WeatherPictureView :weathers="weathers" />
          </div>
...
import WeatherPictureView from './WeatherPictureView.vue';


const weathersColumns = [
  "weatherId",
  "year",
  "date",
  "competitionId",
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
		...
		dataIndex: 'weatherId',
          visible: true,
          scopedSlots: { customRender: 'weatherId' },
          sorter: true
          //sorter: (a, b) => a.weatherId - b.weatherId,
          //sorter: (a, b) => a.weatherId.localeCompare(b.weatherId),
        },
        {
          title: 'City',
		dataIndex: 'city',
      ...
      pagination: {
        ...
        showTotal: (total) => `Total ${total} weathers`,
      },
      ...
      sortBy: 'weatherId',           // Column to sort by
  ...
  methods: {
    ...
    async renderWeathersTable() {
      ...
      let weathersTableData = [];
      for (let i = 0; i < this.weathers.length; i++) {
        weathersTableData.push({
          id: i,
          weatherId: this.weathers[i].weatherId,
          year: this.weathers[i].year,
          date: this.weathers[i].date,
          competitionId: this.weathers[i].competitionId,
          weatherId: this.weathers[i].weatherId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-weathers',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForWeathersChanged', this.searchQuery);
		//this.renderWeathersTable();
    },
	...
	async handleAddSubmitted() {
      ...
      this.weatherToAdd.created = currentDate;
